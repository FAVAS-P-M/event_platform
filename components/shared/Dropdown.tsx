...
getCategories()
    ...
    {categories?.length>0 && categories.map((category)=>(
        <SelectItem key={category._id}
         value={category._id} className="select-auto p-4"  >
            {category.name}
        </SelectItem>
    ))}

<AlertDialog>
  <AlertDialogTrigger className="p-2 flex w-full rounded-sm py-3 pl-8 text-blue-500 hover:bg-blue-50 focus:text-blue-500 text-center justify-center">Add New Category</AlertDialogTrigger>
